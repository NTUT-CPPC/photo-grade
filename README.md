...
- 如果您想要使用 Synology NAS 架設，請參閱 [快速啟動（Docker Compose）](#3-快速啟動docker-compose)
...
只要表頭裡至少出現「**作品名稱** + **作品檔案**」其中一組，或「**作品1_名稱** + **作品1_檔案**」其中一組，就算合法；其他欄位都是選填。系統會「逐列」判斷該列要用哪一種解析方式：**只要該列的 `作品檔案` 有值就用直接模式，否則改看 `作品1_*` / `作品2_*`。** 同一個檔案可以混用兩種風格（每列各自獨立）。

**A. 直接欄位**（推薦給新表單，每列代表一張作品）

| 欄位 | 必要？ | 別名（任一即可） |
| --- | --- | --- |
| `作品名稱` | **必要** | `作品名称`、`Title`、`title`、`name` |
| `作品檔案` | **必要** | `作品档案`、`作品檔案網址`、`Source URL`、`source_url`、`file url`、`image url`、`image_url`、`photo url`、`url` |
| `編號` | 選填 | `投稿編號`、`作品編號`、`ID`、`asset_id`、`entry id`、`submission id` |
| `創作理念` | 選填 | `创作理念`、`Description`、`caption`、`statement`、`concept` |
| `作者` | 選填 | `姓名`、`別稱` |
| `電子郵件地址` | 選填 | `電子郵件`、`Email` |
| `學校` | 選填 | — |
| `系級` | 選填 | `科系`、`年級` |
| `學號` | 選填 | — |

**B. 舊 Google 表單欄位**（每列代表一位投稿者，最多兩張作品）

```
編號, 作品1_名稱, 作品1_檔案, 作品1_創作理念,
作品2_名稱, 作品2_檔案, 作品2_創作理念,
作者, 電子郵件地址, 學校, 系級, 學號
```

每一列代表一位投稿者；作品 2 整段空白會被略過、不會報錯。`作者`、`電子郵件地址`、`學校`、`系級`、`學號` 共用同一份別名規則（同上表）。

`作品1_名稱` 三種寫法都接受、會自動正規化：
...
- 系統會把 `編號` 內所有「非英數字、非 `.` `_` `-` 的字元」（包含中文、空白、`#`、`/` 等）**統一替換成 `-`**，再去掉開頭結尾的 `-`。
- 推薦填**純數字**或**英數字混合**：`123`、`A001`、`2026-001` 都是安全的寫法。
- ⚠️ **不要填純中文編號**（例如 `投稿一`、`組別甲`）：經正規化後會變成空字串，等同沒填編號。

**留空 / 自動編號**

- `編號` 留空時，系統會以**該列在原始檔案中的列序**（第 2 列為 `1`、第 3 列為 `2`、依此類推；標頭算第 1 列）作為編號。
- 注意是「列序」，**不是**「第 N 件作品」。如果中間有純編號的空列被略過（見 §8.4），列序仍然會跳過那一列。

**最終作品代碼會加後綴**

- **直接模式**：每列一張作品，**永遠**加 `a` 後綴。例如 `編號=123` 的列 → 作品代碼 `123a`。要投兩張就用**兩列不同編號**（例如一列 `123`、一列 `124`，得到 `123a`、`124a`）。
- **舊表單模式**：每列最多兩張作品，作品 1 → `a`、作品 2 → `b`。例如 `編號=123` 那列同時填了兩張 → 匯入為 `123a`、`123b`；只填一張也是 `123a`（看不出來他有沒有投第二張）。

> ⚠️ **直接模式下兩列填同一個 `編號` 會被視為「重複作品編號」並擋下 Confirm**，因為兩列都會解析成同一個 `<code>a`。要連投兩件就用不同 `編號`，或改用舊表單模式把兩件填在同一列。
...
- **必要欄位是否齊全**：標頭至少要有 `作品名稱` + `作品檔案`，或舊版的 `作品1_名稱` + `作品1_檔案`（任一組都行）。兩組都沒有 → 整個檔案視為錯誤。
- **連結格式**要是 `http://` 或 `https://`，否則整張作品被略過並顯示 error。
- **重複的作品編號**：同一個 `<code>a` / `<code>b` 出現兩次 → error。
- **名稱缺失**：作品有檔案連結但沒 `作品名稱` → 仍會以「無標題」匯入，但顯示 warning。
- **檔案連結缺失**：作品有名稱但沒檔案連結 → error，這張作品會被略過。

有 error 時 Confirm 按鈕會 disable，請先修檔案再上傳；warning 不擋送出。

> 補充：「**只填 `編號`、其他欄位全空的列**」會被當成 Google 表單導出時的空白列**自動略過、不算作品也不報錯**。如果你發現匯入結果少了幾張，請先檢查那些列除了 `編號` 外是否真的有填內容。
>
> 補充：「**同一列同時填了 `作品檔案` 與 `作品1_檔案`**」時，系統會優先採用直接模式（只匯入一張），舊表單模式的 `作品1_*` / `作品2_*` 會被忽略。建議整份檔案統一用同一種風格，避免混淆。
