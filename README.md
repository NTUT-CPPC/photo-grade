...
| `OIDC_AUTHORIZATION_PARAMS` | _(空)_ | 否 | 額外加到 authorization URL 的 query string，例如 Synology SSO 需 `synossoJSSDK=false`。 |
...
OIDC_AUTHORIZATION_PARAMS=           # 選填；Synology SSO 常用 synossoJSSDK=false
...
| `OIDC_AUTHORIZATION_PARAMS` | 否 | 額外加到 authorization URL 的 query string。Synology SSO 使用手動登入 flow 時請設 `synossoJSSDK=false`。 |
| `OIDC_POST_LOGOUT_REDIRECT_URI` | 否 | 設定後 logout 會把使用者導回此網址（前提是 OP 支援 `end_session_endpoint`）。 |

Synology SSO 範例：

```env
OIDC_ISSUER_URL=https://your-synology.example.com/webman/sso
OIDC_SCOPES=openid email groups
OIDC_AUTHORIZATION_PARAMS=synossoJSSDK=false
```
