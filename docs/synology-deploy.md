...
8. [申請 Let's Encrypt 憑證](#8-申請-lets-encrypt-憑證)
9. [設定反向代理（最重要）](#9-設定反向代理最重要)
10. [建置並啟動](#10-建置並啟動)
11. [（選）用 Synology DSM 本身當 OIDC 提供者](#11-選用-synology-dsm-本身當-oidc-提供者)
12. [日後升級版本](#12-日後升級版本)
13. [備份策略](#13-備份策略)
14. [疑難排解](#14-疑難排解)
...
OIDC 設定細節請看 §11（用 Synology 自己當 OP）或主 [README §6](../README.md#6-使用-oidc-單一登入進階可略)。
...
## 8. 申請 Let's Encrypt 憑證

1. 控制台 → **安全性 → 憑證 → 新增 → 新增憑證**。
2. 選 **「從 Let's Encrypt 取得憑證」** → 下一步。
3. 填：
   - **網域名稱**：`grade.example.com`（你在 §7 設好的網域）
...
點 **設定** 按鈕，找到 `grade.example.com` 那一列，憑證下拉選你剛才申請的 Let's Encrypt 憑證 → 確定。

> 如果這台 NAS 還會跑 DSM 自己的 HTTPS（5001 port）、Drive Server、相片管理等，也可以把它們指到同一張憑證，全部集中管理。
...
- **來源**：
  - 通訊協定：**HTTPS**
  - 主機名稱：`grade.example.com`（與 §7/§8 一致）
  - 連接埠：**443**
  - ✅ 啟用 HSTS（建議）
  - ✅ 啟用 HTTP/2（建議）
- **目的地**：
...
打 https://grade.example.com/api/health 應該回 JSON：

```json
{ "ok": true, "version": "..." }
```

> 還沒啟動容器時會回 502 / 503——下一節會啟動。
...
這就代表 image 拉到、DB schema 推了、server 起來了。worker 那邊應該看到：
...
## 11. （選）用 Synology DSM 本身當 OIDC 提供者
...
按儲存後 **重新部署**（Container Manager 會問你要不要 recreate 容器，按是）。
...
當官方推出新 image（GHCR 上 `:latest` tag 會自動跟著 main 分支更新），你想升級時：

1. **Container Manager → 專案 → photo-grade**。
2. 點 **動作 → 重建**（或 **停止 → 清除 → 建置**，效果一樣）。
3. DSM 會 `docker compose pull` 拉最新 image，再重啟容器。

> ⚠️ **不要**用「重新啟動」——那只是 restart 既有容器，不會去 pull 新 image。
...
`version=` 那段就是 image 帶進來的 commit。如果是 `unknown` 代表你不是在用 GHCR image，是本地 build 的舊版。
...
### 14.1 建置失敗：`Error response from daemon: pull access denied`
...
### 14.3 OIDC 登入卡住 / `{"error":"ERR syntax error"}`
...
- 若是 stopped：點啟動。
- 若一直 restart：看 log，通常與 §14.2 同樣的環境變數錯誤。
...
2. File Station 把 `docker/photo-grade/postgres-data/` 與 `data/` 內容**全刪**（保留資料夾本身）。
3. 回 Container Manager 點 **建置**。

如果只是想保留 DB 但重抓圖片，第 2 步只刪 `data/originals/previews/thumbnails/`。

### 14.7 NAS 重開機後容器沒自動起來
